...
# - The XML files generated by JP101 are moving targets. Errors appear and dissapear, lessons are moved among sections, ... so I can't guarantee a clean result.
...
for my $item ( @{$ref->{'channel'}->{'item'}} )
{
	...
	$title =~ s/^((?:(?:Absolute|Upper|Lower) Beginner)|(?:Audio Blog)|(?:(?:Lower )?Intermediate)|(?:Newbie)|(?:JLPT)|(?:Survival Phrases)) #/$1 S1 #/;
	...
	# 0 - Introduction
		## Japanese Culture Classes
		$title =~ s/^(Japanese Culture Class #4[12]) - Aomori-ben #/$1 - Aomori Dialect #/; # All the subsequent lessons are about "Dialect", not "ben". Also, remove the '#'
	...
	# 1 - Absolute Beginner
		## Kantan Kana
		$title =~ s/^(Kantan Kana #1) - (Lesson Notes|Video)/$1 - Hiragana vowels あ, い, う, え, お - $2/; # Because of old "phantom" entries (with an additional file)
		## Learn with Pictures and Video
		$title =~ s/^(Learn with Pictures and Video) S4/$1/;                                       # There is only one season.
		## Newbie Season 1
		$title =~ s/^Newbie S1 #7 - (Learn to Count in Japanese!)/Newbie S1 #6 - $1/;              # Wrong number # TODO NOGO
		$title =~ s/^Newbie S1 #11 - (Rise and Shine 2)/Newbie S1 #10 - $1/;                       # Wrong number # TODO NOGO
		$title =~ s/^Newbie Lesson S1 #12 - (Winter Blues)/Newbie Lesson S1 #11 - $1/;             # Wrong number # Checked 20150414 GO
		$title =~ s/^Newbie Lesson S1 #13 - (I'm Starving!)/Newbie Lesson S1 #12 - $1/;            # Wrong number # Checked 20150414 GO
		$title =~ s/^Newbie Lesson S1 #14 - (Making the Grade)/Newbie Lesson S1 #13 - $1/;         # Wrong number # Checked 20150414 GO
		$title =~ s/^Newbie Lesson S1 #15 - (When's Your Birthday\?)/Newbie Lesson S1 #14 - $1/;   # Wrong number # Checked 20150414 GO
		$title =~ s/^Newbie Lesson S1 #18 - (To Exist or Not to Exist)/Newbie Lesson S1 #15 - $1/; # Wrong number # Checked 20150414 GO
		## Newbie Season 3
		$title =~ s/^(Newbie Lesson S3 #3 -)(Nihongo Dōjō)/$1 $2/;                                 # Space missing
		## Newbie Season 4
		$title =~ s/^Newbie Lesson #4 - S4:/Newbie S4 #4 -/;                                       # Goddammit JP101. Name mixed up. # TODO NOGO
	...
	# 2 - Beginner
		...
		## Video Vocab Season 1
		$title =~ s/^(Video Vocab Lesson #)/Video Vocab Season 1 #/; # S1 files shouldn't contain "Lesson" (consistency)
	
	# 3 - Intermediate
		## iLove J-Drama
		$title =~ s/^(iLove Chapter [IV]+ - .*? - )iLove -/$1/; # Remove useless "iLove". [IV] because we didn't reach X yet.
		$title =~ s/^(iLove Chapter IV - Nanase's Choice) -$/$1 - (No Subs)/; # Not '- $' because of the trailing space removal
		$title =~ s/^(iLove J-Drama #2 - iLove Chapter II) - ([^i].*)$/$1 - iLove - $2 /; # Oh well. Chapter title "iLove" missing. # Checked 20150414 GO
	...
	# 4 - Advanced
		...
		$title =~ s/^(Advanced Audio Blog) (#35)/$1 S1 $2/; # They forgot to tell it was on S1 # Possible data mix to investigate
		## Advanced Audio Blog 2
		$title =~ s/^(Audio Blog S2 #\d+) - Tokyo Fashon Files/$1 - Tokyo Fashion Files/; # Spelling error (#8 and #11)
		$title =~ s/^(Audio Blog S2 #9 - Tokyo Fashion Files #3 -)/$1 /;                  # Space missing (earlier process changed '#3:Men' into '#3 -Men')
		## Advanced Audio Blog 4
		...
		## (VARIOUS)
		$title =~ s/^(Audio Blog)/Advanced $1/; # They switched to "Advanced" in the middle of S3 # Checked 20150414 GO
	
	# X - Bonus Courses
		## Extra Fun
		$title =~ s/^(Premium Lesson #20 - SS):(16)/$1$2:/; # Misplaced ':'
		$title =~ s/^(Premium Lesson #25 -) (Practicing Priestess)/$1 SS21: $2/;         # They forgot 'SS21:'     # Checked 20150414 GO
		$title =~ s/^(Premium Lesson #26 -) SS23/$1 SS22/;                               # SS22, not SS23          # Checked 20150414 GO
		$title =~ s/^(Premium Lesson #29 -) (Do Not Drink the Water!)/$1 SS25: $2/;      # They forgot 'SS25:'
		$title =~ s/^(Premium Lesson #33 - O-bon Kaidan:)/$1 Stairway In The Middle Of The Night/; # Title missing # Checked 20150414 GO
		## Japanese Children's Songs
		...
		## Inner Circle
		$title =~ s/^(Inner Circle #02)/Inner Circle #2/;                       # Other lessons don't use zeroes   # Checked 20150414 GO
	...
	# Z - News & Announcement
		...
		$title =~ s/^News #91 - JapanesePod101\.com101\.com/News #91 - JapanesePod101.com/;       # Seriously guys? # Checked 20150414 GO
	...
	# X-Files: (when not using hierarchy) (list not up-to-date)
