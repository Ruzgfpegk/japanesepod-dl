...
# You'll need an active membership to JP101 : https://www.japanesepod101.com/
# A free account will let you download the latest 10 audio lessons.
# https://www.japanesepod101.com/helpcenter/getstarted/how-to-save-lessons # Links for the free feed (RSS, you can use it there)
# https://www.japanesepod101.com/helpcenter/getstarted/itunesfeeds         # Links for basic and premium feeds (ITPC only ?)
# https://www.japanesepod101.com/learningcenter/account/myfeed             # Create your own RSS (premium)
#
# Known issues:
# - Windows is NOT supported. This is because many Perl5 modules don't support Unicode file paths under Win32.
...
# - The XML files generated by JP101 are moving targets. Errors appear and disappear, lessons are moved among sections, ...
...
use Time::Piece;              # To parse dates in the XML file (Win: included in Strawberry Perl)
use Unicode::Collate;         # Goddammit Perl. See http://www.perl.com/pub/2011/08/whats-wrong-with-sort-and-how-to-fix-it.html
# We could use File::Spec::Functions to handle file paths as it's included in ActivePerl, but it's not in the OpenSuSE repository.
# Win: included in Strawberry Perl

require LWP::UserAgent;       # To download the files (Win: > cpanm LWP::UserAgent)
require XML::Simple;          # To parse the XML file (Win: included in Strawberry Perl)
...
my %conf = (
	...
	'UserAgent' =>     'iTunes/12.0.1 (Macintosh; OS X 10.9.2) AppleWebKit/537.74.9',
...
# Feel free to update it too, using websites like https://udger.com/resources/ua-list/browser-detail?browser=iTunes
...
croak 'ERROR: Can\'t have a negative waiting time!' if $conf{'wait'} < 0;
...
for my $item ( @{$ref->{'channel'}->{'item'}} )
{
	...
	# $ grep -P "<title>.*THING_YOU_SEARCH.*</title>" JapanesePod101.com-20150411.xml | wc -l
		...
		$title =~ s/\s{2,}/ /g;
		# Remove spaces in between '#' and numbers ('# 2' -> '#2')                             (65 titles on the 20150411 listing)
		$title =~ s/#\s(\d)/#$1/g;
			...
			for my $chapter ( @{$hierarchy{$level}} )
			{
				if( $title =~ /^$chapter .* - .*$/ )
				{
					...
					if( defined $1 && defined $2 ) {
						...
						croak "Lesson or title not found in string ${title2}!";
					}
	...
	if( $path ne '' ) # When hierarchy isn't used or if it failed to create a path
	{
	...
	if( !($conf{'nosummary'}) && $item->{'itunes:summary'} )
	{
		...
		if( !($skip) && ( !(-e $summaryPath) || $conf{'overwrite'} ) ) # If you don't skip and the path doesn't exist or you overwrite...
		{
			...
			my $summary;
			   $summary = IO::File->new( $summaryPath, '>:utf8' ) or croak "Couldn't open $summaryPath for writing: $!";
